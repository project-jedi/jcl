...
{$IFNDEF JCL}
constructor TJclZLibReader.CreateDef2(const Stream: TStream;
  const BufferSize: Integer{$IFDEF SUPPORTS_DEFAULTPARAMS} = JclZLibStreamDefaultBufferSize{$ENDIF});
begin
  Create(Stream, BufferSize, DEF_WBITS);
end;

constructor TJclZLibReader.CreateDef(const Stream: TStream);
begin
  Create(Stream, JclZLibStreamDefaultBufferSize, DEF_WBITS);
end;
...
{$IFNDEF JCL}
constructor TJclZLibWriter.CreateDef2(const Stream: TStream;
  const BufferSize: Integer{$IFDEF SUPPORTS_DEFAULTPARAMS} = JclZLibStreamDefaultBufferSize{$ENDIF};
  const Level: Integer{$IFDEF SUPPORTS_DEFAULTPARAMS} = Z_DEFAULT_COMPRESSION{$ENDIF});
begin
...
constructor TJclZLibWriter.CreateDef(const Stream: TStream);
begin
  Create(Stream, JclZLibStreamDefaultBufferSize, Z_DEFAULT_COMPRESSION,
...
{$IFNDEF JCL}
constructor TJclGZipReader.CreateDef(const Stream: TStream);
begin
  Create(Stream, JclZLibStreamDefaultBufferSize);
end;
...
{$IFNDEF JCL}
constructor TJclGZipWriter.CreateDef2(const Stream: TStream;
  const BufferSize: Integer{$IFDEF SUPPORTS_DEFAULTPARAMS} = JclZLibStreamDefaultBufferSize{$ENDIF};
  const Level: Integer{$IFDEF SUPPORTS_DEFAULTPARAMS} = Z_DEFAULT_COMPRESSION{$ENDIF});
begin
...
constructor TJclGZipWriter.CreateDef(const Stream: TStream);
begin
  Create(Stream, JclZLibStreamDefaultBufferSize, Z_DEFAULT_COMPRESSION,
         Z_DEFAULT_STRATEGY, '', 0, '', False, Nil, 0);
end;
