...
uses
  ...
  JclDebug, JclFileUtils, JclPeImage;
...
begin
  ...
  try
    if AdvBuildFileList(ChangeFileExt(MapFileName, '.*'), faArchive, ExecFilesList, [flFullNames]) then
...
begin
  ...
  try
    if AdvBuildFileList(FileNames, faArchive, FilesList, [flFullNames]) then
...
begin
  WriteLn('Make JCL debug data command line utility. (c) 2001 Project JEDI');
