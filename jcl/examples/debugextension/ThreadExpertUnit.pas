...
uses
  ...
  JclDsgnD4,
  {$ENDIF DELPHI4}
  Windows, Classes, SysUtils, ToolsAPI, ComCtrls, Dialogs, ThreadExpertSharedNames,
  JclSynch;

type
  TNameChangeThread = class;

  TThreadsExpert = class(TNotifierObject, IOTAWizard)
  private
...
procedure TThreadsExpert.Execute;
begin
end;

//------------------------------------------------------------------------------

function TThreadsExpert.GetIDString: String;
begin
  Result := 'JclExtension.ThreadName';
end;

//------------------------------------------------------------------------------

function TThreadsExpert.GetName: String;
begin
  Result := 'ThreadNameExpert';
end;

//------------------------------------------------------------------------------

function TThreadsExpert.GetState: TWizardState;
begin
  Result := [];
end;
...
procedure TThreadsExpert.UpdateItem(Item: TListItem);
var
  TID: DWORD;
...
begin
  ...
  begin
    ...
    ThreadName := FSharedThreadNames.ThreadName[TID];
    if ThreadName <> '' then
    begin
