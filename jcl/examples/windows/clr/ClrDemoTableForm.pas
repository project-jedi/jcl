      ...
      for AKind := Low(TJclClrTableKind) to High(TJclClrTableKind) do
        if Assigned(AStream.Tables[AKind]) then
          ...
          begin
            ...
            SubItems.Add(Copy(AStream.Tables[AKind].ClassName,
              StrLen('TJclClrTable') + 1, MaxWord));
          end;
    finally
      EndUpdate;
    end;
...
procedure TfrmTable.lstTablesSelectItem(Sender: TObject;
...
begin
  for I := 0 to ATable.RowCount - 1 do
    ...
    begin
      ...
      begin
        Dump('%s.%s::%s(%s) @ %p:%d', [ParentToken.Namespace,
          ParentToken.Name, Name, ParamStr, Pointer(RVA), MethodBody.Size]);
        TfrmAbstract.DumpBuf(MethodBody.Code, MethodBody.Size,
          memDump, DWORD(MethodBody.Code), False);
      end
      else
      begin
        Dump('%s.%s::%s(%s)', [ParentToken.Namespace, ParentToken.Name,
          Name, ParamStr]);
      end;
    end;
end;
  ...
  begin
    ...
    if ARow is TJclClrTableParamDefRow then
      with ARow as TJclClrTableParamDefRow do
        Result := Method.ParentToken.Namespace + '.' +
          Method.ParentToken.Name + '::' +
          Method.Name + '(..., ' + Name + ', ...)'
    else
  ...
  begin
    ...
    if AMethod is TJclClrTableMethodDefRow then
      with AMethod as TJclClrTableMethodDefRow do
        Result := ParentToken.Namespace + '.' + ParentToken.Name +
          ' :: ' + Name
    else
