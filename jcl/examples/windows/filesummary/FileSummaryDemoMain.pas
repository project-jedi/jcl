...
procedure TFormMain.UpdateFileSummary(const FileName: string);
  ...
  begin
    if FileTimeToSystemTime(FileTime, ASystemTime) then
      Result := Format('%d/%d/%d %d:%d:%d', [ASystemTime.wYear,
        ASystemTime.wMonth, ASystemTime.wDay,
...
begin
  ...
  try
    FFileSummary.GetPropertySet(TJclFileSummaryInformation,
      AFileSummaryInformation);
    if Assigned(AFileSummaryInformation) then
      try
        ...
        Memo1.Lines.Add('  Edit time ' +
          FileTimeToString(AFileSummaryInformation.EditTime));
        Memo1.Lines.Add('  Last printed time ' +
          FileTimeToString(AFileSummaryInformation.LastPrintedTime));
        Memo1.Lines.Add('  Creation time ' +
          FileTimeToString(AFileSummaryInformation.CreationTime));
        Memo1.Lines.Add('  Last save time ' +
          FileTimeToString(AFileSummaryInformation.LastSaveTime));
        Memo1.Lines.Add('  Page count ' +
          IntToStr(AFileSummaryInformation.PageCount));
        Memo1.Lines.Add('  Word count ' +
          IntToStr(AFileSummaryInformation.WordCount));
        Memo1.Lines.Add('  Char count ' +
          IntToStr(AFileSummaryInformation.CharCount));
      //AFileSummaryInformation.Thumnail
        Memo1.Lines.Add('  App name ' + AFileSummaryInformation.AppName);
        Memo1.Lines.Add('  Security ' +
          IntToStr(AFileSummaryInformation.Security));
      finally
        AFileSummaryInformation.Free;
      end;

    FFileSummary.GetPropertySet(TJclDocSummaryInformation,
    ...
    if Assigned(ADocumentSummaryInformation) then
      try
        ...
        Memo1.Lines.Add('  Pres format ' +
          ADocumentSummaryInformation.PresFormat);
        Memo1.Lines.Add('  Byte count ' +
          IntToStr(ADocumentSummaryInformation.ByteCount));
        Memo1.Lines.Add('  Line count ' +
          IntToStr(ADocumentSummaryInformation.LineCount));
        Memo1.Lines.Add('  Par count ' +
          IntToStr(ADocumentSummaryInformation.ParCount));
        Memo1.Lines.Add('  Slide count ' +
          IntToStr(ADocumentSummaryInformation.SlideCount));
        Memo1.Lines.Add('  Note count ' +
          IntToStr(ADocumentSummaryInformation.NoteCount));
        Memo1.Lines.Add('  Hidden count ' +
          IntToStr(ADocumentSummaryInformation.HiddenCount));
        Memo1.Lines.Add('  MM Clip count ' +
          IntToStr(ADocumentSummaryInformation.MMClipCount));
        Memo1.Lines.Add('  Scale ' +
          BooleanToStr(ADocumentSummaryInformation.Scale));
      ...
      //ADocumentSummaryInformation.DocParts
        ...
        Memo1.Lines.Add('  Links dirty ' +
          BooleanToStr(ADocumentSummaryInformation.LinksDirty));
      finally
        ADocumentSummaryInformation.Free;
      end;

    FFileSummary.GetPropertySet(TJclMediaFileSummaryInformation,
      AMediaFileSummaryInformation);
    ...
    if Assigned(AMSISummaryInformation) then
      try
        ...
        Memo1.Lines.Add('  Restrict ' +
          IntToStr(AMSISummaryInformation.Restrict));
      finally
    ...
    FFileSummary.GetPropertySet(TJclStorageSummaryInformation,
      AFilePropertySet);
    if Assigned(AFilePropertySet) then
      try
    ...
    FFileSummary.GetPropertySet(TJclDisplacedSummaryInformation,
      AFilePropertySet);
    if Assigned(AFilePropertySet) then
      try
    ...
    FFileSummary.GetPropertySet(TJclBriefCaseSummaryInformation,
      AFilePropertySet);
    if Assigned(AFilePropertySet) then
      try
    ...
    FFileSummary.GetPropertySet(TJclWebViewSummaryInformation,
      AFilePropertySet);
    if Assigned(AFilePropertySet) then
      try
    ...
    FFileSummary.GetPropertySet(TJclVideoSummaryInformation,
      AVideoSummaryInformation);
    if Assigned(AVideoSummaryInformation) then
      try
        ...
        Memo1.Lines.Add('  Time length(ms) ' +
          IntToStr(AVideoSummaryInformation.TimeLength));
        Memo1.Lines.Add('  Frame count ' +
          IntToStr(AVideoSummaryInformation.FrameCount));
        Memo1.Lines.Add('  Frame rate ' +
          IntToStr(AVideoSummaryInformation.FrameRate));
        Memo1.Lines.Add('  Data rate ' +
          IntToStr(AVideoSummaryInformation.DataRate));
        Memo1.Lines.Add('  Sample size ' +
          IntToStr(AVideoSummaryInformation.SampleSize));
        Memo1.Lines.Add('  Compression ' + AVideoSummaryInformation.Compression);
        Memo1.Lines.Add('  Stream number ' +
          IntToStr(AVideoSummaryInformation.StreamNumber));
      finally
        AVideoSummaryInformation.Free;
      end;

    FFileSummary.GetPropertySet(TJclAudioSummaryInformation,
    ...
    if Assigned(AAudioSummaryInformation) then
      try
        ...
        Memo1.Lines.Add('  Time length ' +
          IntToStr(AAudioSummaryInformation.TimeLength));
        Memo1.Lines.Add('  Average data rate ' +
          IntToStr(AAudioSummaryInformation.AverageDataRate));
        Memo1.Lines.Add('  Sample rate ' +
          IntToStr(AAudioSummaryInformation.SampleRate));
        Memo1.Lines.Add('  Sample size ' +
          IntToStr(AAudioSummaryInformation.SampleSize));
        Memo1.Lines.Add('  Channel count ' +
          IntToStr(AAudioSummaryInformation.ChannelCount));
        Memo1.Lines.Add('  Stream number ' +
          IntToStr(AAudioSummaryInformation.StreamNumber));
    ...
    FFileSummary.GetPropertySet(TJclControlPanelSummaryInformation,
      AFilePropertySet);
    if Assigned(AFilePropertySet) then
      try
    ...
    FFileSummary.GetPropertySet(TJclVolumeSummaryInformation,
      AFilePropertySet);
    if Assigned(AFilePropertySet) then
      try
