...
{$IFNDEF CLR}
function StrContainsChars(const S: string; Chars: TSysCharSet;
...
function StrPadLeft(const S: string; Len: Integer;
  C: Char = AnsiSpace): string;
function StrPadRight(const S: string; Len: Integer;
  C: Char = AnsiSpace): string;
...
function StrReplaceChars(const S: string; const Chars: TSysCharSet;
  Replace: Char): string;
...
function StrCompareRange(const S1, S2: string;
  const Index, Count: Integer): Integer;
...
function StrHasPrefix(const S: string;
  const Prefixes: array of string): Boolean;
...
function StrMatches(const Substr, S: string;
...
function StrPrefixIndex(const S: string;
  const Prefixes: array of string): Integer;
...
function CharPos(const S: string; const C: Char;
  const Index: Integer = 1): Integer;
function CharLastPos(const S: string; const C: Char;
  const Index: Integer = 1): Integer;
function CharIPos(const S: string; C: Char;
  const Index: Integer = 1): Integer;
...
function StringsToMultiSz(var Dest: PMultiSz;
  const Source: TStrings): PMultiSz;
...
function WideStringsToWideMultiSz(var Dest: PWideMultiSz;
  const Source: TWideStrings): PWideMultiSz;
procedure WideMultiSzToWideStrings(const Dest: TWideStrings;
  const Source: PWideMultiSz);
...
procedure StrIToStrings(S, Sep: string; const List: TStrings;
...
procedure TrimStringsLeft(const List: TStrings;
  DeleteIfEmpty: Boolean = True);
function AddStringToStrings(const S: string; Strings: TStrings;
  const Unique: Boolean): Boolean;
...
procedure StrNormIndex(const StrLen: Integer; var Index: Integer;
...
type
  ...
  private
    ...
    function AppendPChar(Value: PChar; Count: Integer;
      RepeatCount: Integer = 1): TStringBuilder;
    function InsertPChar(Index: Integer; Value: PChar;
      Count: Integer; RepeatCount: Integer = 1): TStringBuilder;
  public
    constructor Create(const Value: string; Capacity: Integer = 16); overload;
    constructor Create(Capacity: Integer = 16; MaxCapacity: Integer = MaxInt);
      overload;
    constructor Create(const Value: string;
      StartIndex, Length, Capacity: Integer); overload;
    destructor Destroy; override;

    function Append(const Value: string): TStringBuilder; overload;
    function Append(const Value: string;
      StartIndex, Length: Integer): TStringBuilder; overload;
    function Append(Value: Boolean): TStringBuilder; overload;
    function Append(Value: Char; RepeatCount: Integer = 1): TStringBuilder;
      overload;
    function Append(const Value: array of Char): TStringBuilder; overload;
    function Append(const Value: array of Char;
      StartIndex, Length: Integer): TStringBuilder; overload;
    ...
    function AppendFormat(const Fmt: string;
      const Args: array of const): TStringBuilder; overload;
    function AppendFormat(const Fmt: string; Arg0: Variant): TStringBuilder;
      overload;
    function AppendFormat(const Fmt: string;
      Arg0, Arg1: Variant): TStringBuilder; overload;
    function AppendFormat(const Fmt: string;
      Arg0, Arg1, Arg2: Variant): TStringBuilder; overload;

    function Insert(Index: Integer; const Value: string;
      Count: Integer = 1): TStringBuilder; overload;
    function Insert(Index: Integer; Value: Boolean): TStringBuilder; overload;
    function Insert(Index: Integer;
      const Value: array of Char): TStringBuilder; overload;
    function Insert(Index: Integer; const Value: array of Char;
      StartIndex, Length: Integer): TStringBuilder; overload;
    ...
    function Replace(OldChar, NewChar: Char; StartIndex: Integer = 0;
      Count: Integer = -1): TStringBuilder; overload;
    function Replace(OldValue, NewValue: string; StartIndex: Integer = 0;
      Count: Integer = -1): TStringBuilder; overload;
    ...
    property __Chars__[Index: Integer]: Char read GetChars write SetChars;
      default;
...
function DotNetFormat(const Fmt: string; const Arg0, Arg1: Variant): string;
  overload;
function DotNetFormat(const Fmt: string;
  const Arg0, Arg1, Arg2: Variant): string; overload;
...
begin
  ...
  begin
    {$IFDEF MSWINDOWS}
    GetStringTypeExA(LOCALE_USER_DEFAULT, CT_CTYPE1, @CurrChar,
      SizeOf(Char), CurrType);
...
begin
  ...
  if (FromIndex <= 0) or (FromIndex > Length(Source)) or
    ...
    ((FromIndex + Count - 1) > Length(Source)) or
    ((ToIndex + Count - 1) > Length(Dest)) then
...
var
 ...
 { pointers into S and Search when first character has }
  SearchMatchPtr: PChar;
 { been matched and we're probing for a complete match }
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if IgnoreCase then
          while (CharUpper(SourceMatchPtr^) = SearchMatchPtr^) and
            (SearchMatchPtr^ <> #0) do
          begin
            Inc(SourceMatchPtr);
            Inc(SearchMatchPtr);
          ...
          while (SourceMatchPtr^ = SearchMatchPtr^) and
            (SearchMatchPtr^ <> #0) do
          begin
            Inc(SourceMatchPtr);
            Inc(SearchMatchPtr);
...
begin
  ...
  begin
    First := S[1];
    Last := S[L];
    if (First = Last) and ((First = AnsiSingleQuote) or
      (First = AnsiDoubleQuote)) then
      Result := Copy(S, 2, L - 2)
    else
      Result := S;
  end
...
{$ELSE}
function StrCompareRange(const S1, S2: string;
  const Index, Count: Integer): Integer; assembler;
asm
  TEST    EAX, EAX
  JZ      @@Str1Null
...
asm
  ...
  CMP     AL, [ESI + EDX]
  JNE     @@FindNext
// if still no, go back to the main loop
...
begin
  ...
  PosEnd := StrSearch(Stop, S, PosStart + 1);
  // PosEnd has to be after PosStart.
...
function StringsToStr(const List: TStrings; const Sep: string;
  const AllowEmptyString: Boolean): string;
var
  I, L: Integer;
begin
  Result := '';
...
function DotNetFormat(const Fmt: string;
  const Arg0, Arg1, Arg2: Variant): string;
begin
  ...
  begin
    case V.VType of
      ...
      varByRef:}
        ...
        raise ArgumentNullException.CreateResFmt(
          @RsDotNetFormatArgumentNotSupported, [Index]);
    end;
  end;
  ...
  begin
    ...
    case V.VType of
      ...
      vtPChar:
        Result := V.VPChar;
      vtObject:
        if (V.VObject is TInterfacedObject) and
          V.VObject.GetInterface(IToString, Intf) then
        begin
        ...
        if InheritsFrom(V.VObject.ClassType, 'TComponent') and
          V.VObject.GetInterface(IToString, Intf) then
          Result := Intf.ToString
        else
          raise ArgumentNullException.CreateResFmt(
...
function TStringBuilder.AppendPChar(Value: PChar; Count: Integer;
...
function TStringBuilder.Append(const Value: array of Char;
...
function TStringBuilder.Append(Value: Char;
  RepeatCount: Integer = 1): TStringBuilder;
begin
...
function TStringBuilder.Append(const Value: string;
  StartIndex, Length: Integer): TStringBuilder;
var
  Len: Integer;
begin
...
function TStringBuilder.AppendFormat(const Fmt: string;
  Arg0: Variant): TStringBuilder;
begin
  Result := Append(DotNetFormat(Fmt, [Arg0]));
end;

function TStringBuilder.AppendFormat(const Fmt: string;
  Arg0, Arg1: Variant): TStringBuilder;
begin
  Result := Append(DotNetFormat(Fmt, [Arg0, Arg1]));
end;

function TStringBuilder.AppendFormat(const Fmt: string;
  Arg0, Arg1, Arg2: Variant): TStringBuilder;
begin
...
function TStringBuilder.Insert(Index: Integer;
  const Value: array of Char): TStringBuilder;
var
  Len: Integer;
begin
  Len := System.Length(Value);
...
function TStringBuilder.Replace(OldValue, NewValue: string;
...
begin
  ...
  begin
    ...
    for I := StartIndex to StartIndex + Length - 1 do
      ...
      begin
        if OldLen > 1 then
          if StrLComp(@FChars[I + 1], PChar(OldValue) + 1,
            OldLen - 1) <> 0 then
            Continue;
        if Offset <> 0 then
        begin
          ...
          if Offset < 0 then
            ...
            MoveChar(FChars[I + OldLen], FChars[I + OldLen + Offset],
              FLength - OldLen - I);
          Inc(FLength, Offset);
        end;
        if NewLen > 0 then
