...
function PeInsertSection(const FileName: TFileName; SectionStream: TStream; SectionName: string): Boolean;
  ...
  begin
    ...
    try
      ...
      {$IFDEF RTL250_UP}System.AnsiStrings.{$ENDIF}StrPLCopy(PAnsiChar(@NewSection^.Name), UTF8Name, IMAGE_SIZEOF_SHORT_NAME);
...
begin
  ...
  begin
    ...
    for I := 1 to NtHeaders^.FileHeader.NumberOfSections do
      if {$IFDEF RTL250_UP}System.AnsiStrings.{$ENDIF}StrLComp(PAnsiChar(@Header^.Name), P, IMAGE_SIZEOF_SHORT_NAME) = 0 then
      begin
        Result := Header;
        Break;
      end
...
begin
  ...
  begin
    ...
    for Result := Low(ImageSectionHeaders) to High(ImageSectionHeaders) do
      if {$IFDEF RTL250_UP}System.AnsiStrings.{$ENDIF}StrLComp(PAnsiChar(@ImageSectionHeaders[Result].Name), P, IMAGE_SIZEOF_SHORT_NAME) = 0 then
        Exit;
  end;
  Result := -1;
end;
...
begin
  ...
  begin
    ...
    if {$IFDEF RTL250_UP}System.AnsiStrings.{$ENDIF}StrIComp(CurrName, RefName) = 0 then
    begin
      {$IFDEF CPU32}
...
var
  ...
  procedure ReadRTTI;
  begin
    if {$IFDEF RTL250_UP}System.AnsiStrings.{$ENDIF}StrLComp(NameP, '$xp$', 4) = 0 then
    begin
      Inc(NameP, 4);
...
begin
  ...
  NameU^ := #0;
  SetLength(UTF8Unmangled, {$IFDEF RTL250_UP}System.AnsiStrings.{$ENDIF}StrLen(PAnsiChar(UTF8Unmangled))); // SysUtils prefix due to compiler bug
