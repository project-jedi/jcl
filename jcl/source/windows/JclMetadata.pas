...
type
  ...
  TJclClrTableFieldVisibility = (fvPrivateScope, fvPrivate, fvFamANDAssem,
                                 fvAssembly, fvFamily, fvFamORAssem, fvPublic);

  TJclClrTableFieldFlag = (ffStatic, ffInitOnly, ffLiteral, ffNotSerialized,
                           ffSpecialName, ffPinvokeImpl, ffRTSpecialName,
                           ffHasFieldMarshal, ffHasDefault, ffHasFieldRVA);
  TJclClrTableFieldFlags = set of TJclClrTableFieldFlag;
  ...
  private
    ...
    function GetFlag: TJclClrTableFieldFlags;
    function GetVisibility: TJclClrTableFieldVisibility;
  protected
    constructor Create(const ATable: TJclClrTable); override;
  ...
  public
    ...
    property Visibility: TJclClrTableFieldVisibility read GetVisibility;
    property Flags: TJclClrTableFieldFlags read GetFlag;
  end;
  ...
  private
    ...
    function GetProperty(const Idx: Integer): TJclClrTablePropertyRow;
    function GetPropertyCount: Integer;
  protected
    constructor Create(const ATable: TJclClrTable); override;

    function Add(const ARow: TJclClrTablePropertyRow): Integer;
  public
    destructor Destroy; override;
    ...
    property Properties[const Idx: Integer]: TJclClrTablePropertyRow read GetProperty;
...
begin
  ...
  else if (pData[0] and $C0) = $80 then // 10?? ????
  begin
    Value  := (pData[0] and $3F) shl 8 + pData[1];
    Result := 2;
  end
  else if (pData[0] and $E0) = $C0 then // 110? ????
  begin
...
const
  ...
  TypedTypeName: array[etPtr..etClass] of string =
...
begin
  case ElementType of
    etBoolean: Result := BoolToStr(PBoolean(Value.Memory)^, True);
    etChar:    Result := PWideChar(Value.Memory)^;
    etI1:      Result := IntToStr(PShortInt(Value.Memory)^);
    etU1:      Result := IntToStr(PByte(Value.Memory)^);
    etI2:      Result := IntToStr(PSmallint(Value.Memory)^);
    etU2:      Result := IntToStr(PWord(Value.Memory)^);
    etI4:      Result := IntToStr(PInteger(Value.Memory)^);
    etU4:      Result := IntToStr(PDWord(Value.Memory)^);
    etI8:      Result := IntToStr(PInt64(Value.Memory)^);
    etU8:      Result := IntToStr(PInt64(Value.Memory)^);
    etR4:      Result := FloatToStr(PSingle(Value.Memory)^);
    etR8:      Result := FloatToStr(PDouble(Value.Memory)^);
    etString:  Result := '"' + WideCharLenToString(PWideChar(Value.Memory), Value.Size div 2) + '"';
    etClass:
    begin
...
begin
  ...
  Result := Table.Stream.Tables[HasConstantMapping[FParentIdx and 3]].Rows[FParentIdx shr 2];
end;
...
function TJclClrTableFieldRow.DumpIL: String;
  function DumpFlags: string;
  const
    FlagName: array[ffInitOnly..ffRTSpecialName] of string =
    ('initonly', 'literal', 'notserialized', 'specialname', '', '');
  var
    AFlag: TJclClrTableFieldFlag;
  begin
...
const
  StaticName: array[Boolean] of string = ('', 'static ');
  VisibilityName: array[TJclClrTableFieldVisibility] of string =
...
begin
  ...
  else if ffHasFieldRVA in Flags then
  begin
  end;
end;
...
function TJclClrTableFieldRow.GetVisibility: TJclClrTableFieldVisibility;
const
  FieldVisibilityMapping: array[fdPrivateScope..fdPublic] of TJclClrTableFieldVisibility =
...
function TJclClrTableFieldRow.GetFlag: TJclClrTableFieldFlags;
const
  FieldFlagMapping: array[TJclClrTableFieldFlag] of Word =
...
var
  AFlag: TJclClrTableFieldFlag;
begin
  for AFlag:=Low(TJclClrTableFieldFlag) to High(TJclClrTableFieldFlag) do
...
begin
  ...
  else if ImplInterface is TJclClrTableTypeDefRow then
...
const
  VisibilityName: array[TJclClrTableManifestResourceVisibility] of string = ('public', 'private');
var
  I: Integer;
  tblCustomAttribute: TJclClrTableCustomAttribute;
begin
...
begin
  for Result:=Low(TJclClrTableManifestResourceVisibility) to High(TJclClrTableManifestResourceVisibility) do
    if (FFlags and mrVisibilityMask) = ManifestResourceVisibilityMapping[Result] then
      Exit;
  raise Exception.CreateFmt('Unknonw manifest resource visibility - %d', [FFlags and mrVisibilityMask]);
end;
...
function TJclClrTablePropertyRow.DumpIL: String;
  function DumpFlags: string;
  const
...
function TJclClrTablePropertyRow.GetFlags: TJclClrTablePropertyFlags;
var
...
function TJclClrTablePropertyMapRow.Add(const ARow: TJclClrTablePropertyRow): Integer;
begin
  Result := FProperties.Add(ARow);
end;

function TJclClrTablePropertyMapRow.GetProperty(const Idx: Integer): TJclClrTablePropertyRow;
begin
  Result := TJclClrTablePropertyRow(FProperties.Items[Idx]);
end;
...
begin
  J := 0;
  with TJclClrTableProperty(Stream.Tables[ttPropertyDef]) do
  for I:=0 to RowCount-1 do
  begin
...
const
  ClassSemanticName: array[TJclClrClassSemantics] of string =
  ('class', 'interface');
  VisibilityName: array[TJclClrTypeVisibility] of string =
  ('private', 'public', 'nested public', 'nested private',
   'nested family', 'nested assembly', 'nested famandassem', 'nested famorassem');
  ClassLayoutName: array[TJclClrClassLayout] of string =
  ('auto', 'explicit', 'sequential');
  StringFormattingName: array[TJclClrStringFormatting] of string =
  ('ansi', 'unicode', 'autoChar');
  TypeAttributeName: array[TJclClrTypeAttribute] of string =
...
begin
  ...
  try
    ...
    try
      with Table.Stream.Tables[ttInterfaceImpl] do
      for I:=0 to RowCount-1 do
        if TJclClrTableInterfaceImplRow(Rows[I]).ClassIdx = DWORD(Index + 1) then
          lstIntfs.Add(TJclClrTableInterfaceImplRow(Rows[I]));
...
begin
  ...
  else if ResolutionScope is TJclClrTableModuleRefRow then
    Result := TJclClrTableModuleRefRow(ResolutionScope).Name
  else if ResolutionScope is TJclClrTableAssemblyRefRow then
    Result := TJclClrTableAssemblyRefRow(ResolutionScope).Name
  else if ResolutionScope is TJclClrTableTypeRefRow then
